...
class Datacenter():
    ...
    def parallelizedFunc(self, IP):
        ...
        resp = requests.post("http://" + IP + ":8081/request", data=json.dumps(payload), headers=headers)
    ...
    def generateHosts(self):
        ...
        for i, data in enumerate(outputHostsData):
            ...
            Ram = RAM(data['totalMemmory'], data['ramRead'], data['ramWrite'])
            Disk_ = Disk(data['totalDisk'], data['diskRead'], data['diskWrite'])
