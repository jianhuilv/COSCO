...
def setupVLANEnvironment(cfg, mode):
    ...
    for ip in HOST_IPS:
        res = os.system(
            "ssh " + uname + "@" + ip + " sh digitalTwin/scripts/delete.sh")# > ./dev/eventLog.log 2>&1")

        res = os.system(
            "ssh " + uname + "@" + ip + " sudo service docker restart ") #> ./dev/eventLog.log 2>&1")
...
def setupVagrantEnvironment(cfg, mode):
    ...
    uname = "vagrant"
    for ip in HOST_IPS:
        res = call(["ssh", "-o", "StrictHostKeyChecking=no", "-i", "framework/install_scripts/ssh_keys/id_rsa",
                    uname + "@" + ip, "~/agent/scripts/delete.sh"], shell=True, stdout=PIPE, stderr=PIPE)
    return HOST_IPS
